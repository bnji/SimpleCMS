...
namespace General.Helpers
{
    ...
    {
        #region Singleton
        private static CultureHelper _instance;
        ...
        public string GetImplementedCulture(string name)
        {
            ...
            if (_cultures.Where(c => c.Key.Equals(name, StringComparison.InvariantCultureIgnoreCase)).Count() > 0)
                return name; // accept it
            // Find a close match. For example, if you have "en-US" defined and the user requests "en-GB", 
            // the function will return closes match that is "en-US" because at least the language is the same (ie English)  
            var n = GetNeutralCulture(name);
            foreach (var c in _cultures)
                if (c.Key.StartsWith(n))
                    return c.Key;
            // else 
        ...
        {
            get { return _cultures.First().Key; } // return Default culture
        }
        ...
        {
            var culture = System.Web.HttpContext.Current.Request.Cookies.Get("_culture");
            var cultureName = "";
            if (culture != null)
            {
        ...
        public static string GetCurrentUICultureLowerInvariant()
        {
