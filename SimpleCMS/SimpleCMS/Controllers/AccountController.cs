...
namespace BootstrapControllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            return SimpleCMS.BLL.AccountHandler.Instance.Initialize(User.Identity.Name);
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", @SimpleCMS.SimpleCMS.Resources.Blog.Resources.InvalidUsernameOrPassword);
                }
            }
        ...
        void AddUserToRole(ApplicationUser user, string roleName = "User")
        {
            ...
            if (ModelState.IsValid)
            {
                using (var db = new ApplicationDbContext())
                {
                    if (db.Users.Count() > 0)
                    {
                        ...
                        var adminRole = db.Roles.Where(x => ("" + x.Name).ToLower().Equals("admin")).FirstOrDefault();
                        var isAdmin = false;
                        if (currentUser != null && adminRole != null)
                        {
                            var adminUser = currentUser.Roles.Where(x => x.RoleId == adminRole.Id).FirstOrDefault();
                            isAdmin = adminUser != null;
                        }
                        if (!isAdmin)
                        {
                            ModelState.AddModelError("", "You don't have administrator rights!");
                            return View(model);
                        }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", @SimpleCMS.SimpleCMS.Resources.Blog.Resources.UserDoesntExistOrNotConfirmed);
                    return View();
                }
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null)
                {
                    ModelState.AddModelError("", @SimpleCMS.SimpleCMS.Resources.Blog.Resources.NoUserFound);
                    return View();
                }
        ...
        public ActionResult ChangePassword(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePasswordSuccess ? @SimpleCMS.SimpleCMS.Resources.Blog.Resources.YourPasswordHasBeenChanged
                : message == ManageMessageId.SetPasswordSuccess ? @SimpleCMS.SimpleCMS.Resources.Blog.Resources.YourPasswordHasBeenSet
                : message == ManageMessageId.RemoveLoginSuccess ? @SimpleCMS.SimpleCMS.Resources.Blog.Resources.TheExternalLoginWasRemoved
                : message == ManageMessageId.Error ? @SimpleCMS.SimpleCMS.Resources.Blog.Resources.AnErrorHasOccurred
                : "";
